...
import { filter, map, mergeMap, skipUntil, startWith, take, tap } from 'rxjs/operators';
...
export class SourceComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    let menu = fromEvent(this.socket.nativeElement, 'mousedown').pipe(
    ...
    this.menu = menu.subscribe(() => this.menuActive = true);
